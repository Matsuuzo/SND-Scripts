...
-- Character list with assigned helpers
-- Format: {"Inviting Char@World", "Helper 1", "Helper 2", "Helper 3"}
-- • Helper 1: First helper character (Account 2)
-- • Helper 2: Second helper character (Account 3)
-- • Helper 3: Third helper character (Account 4)
-- • Use "" (empty string) to skip verification for any helper position
-- • Helpers can be the same across characters (static) or different (rotating)


local charConfigs = {
    {{"Inviting CharOne@World"}, "Helper One", "Helper Two", "Helper Three"},
    {{"Inviting CharTwo@World"}, "Helper One", "Helper Two", "Helper Three"},
    {{"Inviting CharThree@World"}, "Helper One", "Helper Two", "Helper Three"} 
}
...
local originalCharForSubmarines = nil
...
local currentHelper2 = charConfigs[1] and charConfigs[1][3] and tostring(charConfigs[1][3]) or ""
local currentHelper3 = charConfigs[1] and charConfigs[1][4] and tostring(charConfigs[1][4]) or ""
local idx = 1
...
local function IsHelperInParty(helperName)
    if not helperName or helperName == "" then
        return true  -- Skip verification if no name specified
    end
...
local function IsPartyComplete(helper1, helper2, helper3)
    local partySize = Svc.Party.Length
...
local function ResetRotation()
    ...
    if charConfigs[1] and charConfigs[1][1] and charConfigs[1][1][1] then
        ...
        currentHelper1 = charConfigs[1][2] and tostring(charConfigs[1][2]) or ""
...
local function switchToNextCharacter()
    ...
    if attemptCharacterLogin(nextIdx) then
        ...
        if charConfigs[nextIdx] and charConfigs[nextIdx][1] and charConfigs[nextIdx][1][1] then
            ...
            currentHelper2 = charConfigs[nextIdx][3] and tostring(charConfigs[nextIdx][3]) or ""
...
while rotationStarted do
    ...
    -- Duty Verification - Bei Fehler trotzdem weiter
    local checkSuccess, rewardStatus = CheckDutyRouletteReward()
